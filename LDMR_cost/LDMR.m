%function [Xn] = LDMR(y, A, D, trls, Xinit, Einit, alpha, beta, imgsize,c)
function [Xn] = LDMR(y, A, D, trls, Xinit, Einit, alpha, beta, imgsize,No_p,ttls)
mu = 1;
eps_abs = 1e-3;
eps_rel = 1e-3;
Max_Iter = 100;
...
for iter = 1:Max_Iter  
    ...
    % update E
   % c = cost(ttls,Xo, A, No_p,trls,imgsize);
  %  m1 = reshape( c.*(A*Xo - y + Zo/mu), imgsize);
    m1 = reshape( cost(Xo, A, No_p,trls,imgsize,ttls).* (A*Xo - y + Zo/mu), imgsize);
 ...
 %   En = c.*AU(:,1:SVP)*diag(SU)*VU(:,1:SVP)';
    ...
    g = y + En - Zo/mu;
    Xn = M*g; 
 %   c = cost(ttls,X, trdat, i, trls,imgsize);
 %   Xn = c.*M*g; 
